...
def regra_atualizar_estado_voo(sistema):
    ...
    sistema['obstaculo_y'] += OBSTACULO_VELOCIDADE * \
        sistema['obstaculo_direcao']
...
def regra_mover_drone(sistema):
    ...
    if distancia >= velocidade:
        ...
        if condicao_verifica_colisao(x1, y1, novo_x, novo_y, sistema['obstaculo_y']):
            if not sistema['evitando_obstaculo']:
...
def condicao_verifica_colisao(x1, y1, x2, y2, obstaculo_y):
    obstaculo_x = LARGURA // 2
    colisao = False
    if min(x1, x2) <= obstaculo_x <= max(x1, x2):
        if x1 != x2:
            t = (obstaculo_x - x1) / (x2 - x1)
            y_intersecao = y1 + t * (y2 - y1)
            if obstaculo_y <= y_intersecao <= obstaculo_y + OBSTACULO_ALTURA:
                colisao = True
    return colisao
...
def regra_calcular_rota_alternativa(sistema, x1, y1, x2, y2):
    ...
    # Drone está à direita do obstáculo
    ponto_desvio = (obstaculo_x + OBSTACULO_LARGURA + 10,
                    min(obstaculo_y - 20, max(y1, y2)))

    if x1 < obstaculo_x:
        # Drone está à esquerda do obstáculo
        ponto_desvio = (obstaculo_x - OBSTACULO_LARGURA - 10,
                        min(obstaculo_y - 20, max(y1, y2)))
...
def desenhar_sistema(sistema):
    ...
    # Desenhar obstáculo
    pygame.draw.rect(tela, AMARELO, (LARGURA // 2 - OBSTACULO_LARGURA //
                     2, sistema['obstaculo_y'], OBSTACULO_LARGURA, OBSTACULO_ALTURA))

    # Desenhar drone
    pygame.draw.circle(tela, VERMELHO, (int(
        sistema['posicao_drone'][0]), int(sistema['posicao_drone'][1])), 10)

    # Desenhar informações
    fonte_info = pygame.font.Font(None, 24)
    info = f"Estado: {sistema['estado_drone']} | Posição Atual: {
        sistema['posicao_atual']} | Próximo Destino: {sistema['proximo_destino']}"
