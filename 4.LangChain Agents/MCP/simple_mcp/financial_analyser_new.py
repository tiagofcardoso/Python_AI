...
class FinancialAnalyser:
    ...
    def process_query(self, user_query: str) -> Dict[str, Any]:
        ...
        if "database_query" in llm_data and llm_data["database_query"]:
            ...
            if "sql_query" in llm_data:
                ...
                if ticker:
                    # Usar uma consulta SQL direta em vez de chamar um método inexistente
                    sql_query = f"SELECT TOP 1 Ticker, Data, Alta AS MelhorPreco FROM HistoricoAcoes WHERE Ticker = '{ticker}' ORDER BY Alta DESC"
                    return self.server.execute_db_query(sql_query)
            elif "worst_price" in llm_data:
                # Obter o pior preço de uma ação
                ticker = llm_data["worst_price"].get("ticker")
                if ticker:
                    # Usar uma consulta SQL direta em vez de chamar um método inexistente
                    sql_query = f"SELECT TOP 1 Ticker, Data, Baixa AS PiorPreco FROM HistoricoAcoes WHERE Ticker = '{ticker}' ORDER BY Baixa ASC"
                    return self.server.execute_db_query(sql_query)
    ...
    def format_result(self, result: Dict[str, Any]) -> str:
        ...
        if "recommendation" in result:
            ...
            output = []
            output.append(
                f"Análise da ação {result['ticker']} (período: {result['period']})")
            output.append(f"Preço médio: {result['metrics']['avg_price']:.2f}")
            output.append(
                f"Preço máximo: {result['metrics']['max_price']:.2f}")
            if 'max_price_date' in result['metrics']:
                output.append(
                    f"Data do preço máximo: {result['metrics']['max_price_date']}")
            output.append(
                f"Preço mínimo: {result['metrics']['min_price']:.2f}")
            if 'min_price_date' in result['metrics']:
                output.append(
                    f"Data do preço mínimo: {result['metrics']['min_price_date']}")
            output.append(f"Desvio padrão: {result['metrics']['std_dev']:.2f}")
            output.append(
                f"Volume médio: {result['metrics']['volume_avg']:.2f}")
            output.append(f"Recomendação: {result['recommendation']}")
            output.append(
                f"Baseado em {result['data_points']} pontos de dados")
            return "\n".join(output)
        ...
        if "data" in result and result["data"]:
            output.append(
                f"Encontrados {result.get('count', len(result['data']))} registros:")

            # Mostrar apenas os primeiros 5 resultados para não sobrecarregar o terminal
            for i, row in enumerate(result["data"][:5]):
                output.append(f"\nRegistro {i+1}:")
                for key, value in row.items():
                    # Formatação especial para melhor/pior preço
                    if key == "MelhorPreco":
                        output.append(f"  Melhor Preço: {value}")
                    elif key == "PiorPreco":
                        output.append(f"  Pior Preço: {value}")
                    elif key == "Data" and ("MelhorPreco" in row or "PiorPreco" in row):
                        output.append(f"  Data: {value}")
                    else:
